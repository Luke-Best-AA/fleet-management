...
async def vehicle_delete_post(request: Request, vehicle_id: int, db: Session = Depends(get_db)):
    ...
    audit_service.log_action(db, user_id=user["id"], action="delete", target_type="vehicle", target_id=vehicle_id)
...
async def vehicle_unretire_post(request: Request, vehicle_id: int, db: Session = Depends(get_db)):
    ...
    audit_service.log_action(db, user_id=user["id"], action="unretire", target_type="vehicle", target_id=vehicle_id)
