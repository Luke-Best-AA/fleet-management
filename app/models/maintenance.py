...
class MaintenanceCategory(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        UniqueConstraint("name", name="uq_maintenance_categories_name"),
    )
...
class MaintenanceRecord(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __table_args__ = (
        CheckConstraint(
            "mileage_at_time >= 0", name="ck_maintenance_records_mileage_at_time"
        ),
        CheckConstraint(
            "cost IS NULL OR cost >= 0", name="ck_maintenance_records_cost"
        ),
    )
