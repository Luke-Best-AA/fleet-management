...
class TestGetRecordsForVehicle:
    ...
    def test_excludes_deleted(self, db, vehicle, admin_user):
        rec = mileage_service.create_record(db, vehicle.id, admin_user.id, 15000, user_role="admin", user_id=admin_user.id)
        rec.is_deleted = True
        db.commit()
        records = mileage_service.get_records_for_vehicle(db, vehicle.id)
...
class TestCreateMileageRecord:
    ...
    def test_standard_user_unassigned_vehicle_fails(self, db, vehicle, location):
        ...
        other = user_service.create_user(
            db, username="other_driver", email="other@test.com",
            password="password123", first_name="Other", last_name="Driver",
            location_id=location.id,
        )
        with pytest.raises(AuthorisationError):
            mileage_service.create_record(
                db, vehicle.id, other.id, 20000,
    ...
    def test_standard_user_override_ignored(self, db, vehicle, standard_user):
        rec = mileage_service.create_record(
            db, vehicle.id, standard_user.id, 15000,
            is_admin_override=True, override_reason="Trying override",
            user_role="standard", user_id=standard_user.id,
        )
    ...
    def test_admin_override_with_reason_succeeds(self, db, vehicle, admin_user):
        rec = mileage_service.create_record(
            db, vehicle.id, admin_user.id, 5000,
            is_admin_override=True, override_reason="Odometer correction",
            user_role="admin", user_id=admin_user.id,
        )
    ...
    def test_updates_current_mileage_on_higher(self, db, vehicle, admin_user):
        mileage_service.create_record(
            db, vehicle.id, admin_user.id, 25000,
            user_role="admin", user_id=admin_user.id,
        )
        db.refresh(vehicle)
    ...
    def test_admin_override_recalculates_mileage(self, db, vehicle, admin_user):
        ...
        mileage_service.create_record(
            db, vehicle.id, admin_user.id, 5000,
            is_admin_override=True, override_reason="Reset",
            user_role="admin", user_id=admin_user.id,
        )
        db.refresh(vehicle)
